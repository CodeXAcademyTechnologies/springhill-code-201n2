...
CookieStand.prototype.randomRate = function () {
  for (let i = 0; i < (this.closeTime - this.openTime); i++) {
    ...
    console.log(this.aveCookies, this.max, this.min);
  ...
  for (let i in this.cookieSoldArray) {
    ...
    console.log(this.cookieSoldTotal)
  }
...
function renderTable() {
  ...
  for (let i in state.allCookieStands) {
    console.log(state.allCookieStands[i]);
...
function handleLocationSubmit(event) {
  ...
  if (state.allLocationsRef.indexOf(name) > -1) {
    ...
    state.allCookieStands[state.allLocationsRef.indexOf(name)].randomRate();
    renderTable();
    sumColumns();
    return;
  }
